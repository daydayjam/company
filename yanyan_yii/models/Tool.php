...
class Tool {
     ...
     */
    public static function connectPath($domain, $path, $default = '') {
        $newStr = $path;
        if(!empty($path) && !self::startWith($path, 'http')) {
            $newStr = $domain . $path;
        } else if(empty($path) && !$default) {
            $newStr = $domain . $default;
        }
        if(($index = strpos($newStr, 'pic=')) !== false) {
            $newStr = substr($newStr, $index+4);
        }
        if($httpsIndex = strpos($newStr, 'https:http') === 0) {
            $newStr = substr($newStr, $index+6);
        }
        return $newStr;
    }
     ...
     * @param type $type
     */
    public static function getUniqueValue($tel, $code, $type) {
        return 'yanyan926_' . $tel . '_' . $code . '_' . $type;
    }
    ...
    */
     ...
     * @param type $cookie
     */
    public static function phpCurl($url, $cookie = '', $method = 'get') {
        $ch = curl_init();  
    ...
    public static function dateFormat($date) {
        ...
        if(strpos($date, '分钟') > 0 || strpos($date, '小时') > 0 || $date == '刚刚') {
            return date('Y-m-d');
        }
     ...
     * @return type
     */
    public static function trimMark($str) {
        $char = "·?(),，。!、！？：;．. :；﹑•＂…‘’“”〝〞∕¦‖—〈〉﹞﹝「」‹›〖〗】【»«』『〕〔》《﹐¸﹕︰﹔！¡？¿﹖﹌﹏﹋＇´ˊˋ―﹫︳︴¯＿￣﹢﹦﹤‐­˜﹟﹩﹠﹪﹡﹨﹍﹉﹎﹊ˇ︵︶︷︸︹︿﹀︺︽︾ˉ﹁﹂﹃﹄︻︼（）～（）#";
        $reg = "/[[:punct:]]|['.$char.']|[ ]{1,}|\s/iu";
//        print_r($str);
//        $ss = preg_replace($reg, '', $str);
        $ss = mb_ereg_replace($reg, '', $str); 
//        print_r($ss);die;
        return $ss;
    }
