...
class Easemob{
    ...
    private $client_id = "YXA65X5x8OZmEeWQYVPt9YzNrg";
    private $client_secret = "YXA6ml9d8kfCOOi9YZXsyqLw-UeLqaU";
    private $org_name = "crting-chat";
    private $app_name = "crtingchat";
    private $url;
...
//		$this->app_name = isset ( $options ['app_name'] ) ? $options ['app_name'] : '';
    ...
    */
    function getToken()
    {
        ...
        //return $tokenResult;
        return "Authorization:Bearer ".$tokenResult['result']['access_token'];
    ...
    */
    function getChatRecordForPage( $ql,$limit=0,$cursor = ""){
        ...
        $header=array( $this->getToken());
    //		echo "url=".$url."<br/>token=".$header[0]."<br/>";
        $result=$this->postCurl( $url,'',$header,"GET" );
        $cursor=$result["cursor"];
     ...
     *$this->postCurl方法
     */
    function postCurl( $url,$body,$header,$type="POST", $needGetToken = true){
        //1.创建一个curl资源
        $ch = curl_init();
        //2.设置URL和相应的选项
        ...
        if(empty( $result)){
            ...
            if(isset($result["data"]) && isset($result["data"]["result"]) && $result["data"]["result"] == false){
                $httpCode = -1;//执行失败
            }
        }
        $retValue["code"] = $httpCode;
        if($httpCode == 401 && $needGetToken){
            return $this->postCurl($url,$body,$header,$type, false);
        }
