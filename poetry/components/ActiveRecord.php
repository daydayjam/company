...
class ActiveRecord extends \yii\db\ActiveRecord {
    ...
    public function insert($runValidation = true, $attributeNames = null) {
        if(!isset($this->update_time) || $this->update_time == 0) {
            $this->update_time = time();
        }
        if($attributeNames == null){
            if(!isset($this->create_time) || $this->create_time==0){
                $this->create_time = time();
            }
        } else if(!isset($attributeNames['create_time']) || $this->create_time==0){
            $attributeNames['create_time'] = time();
        }
