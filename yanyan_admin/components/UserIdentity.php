...
class UserIdentity {
    ...
    public function authenticate() {
        ...
        if($this->_type == 1) { //常规登陆
            $sql = 'select * from ' . $User->tableName() . ' where tel=:tel or serial_num=:tel';
            $UserRecord = $User->findBySql($sql, [':tel'=>$this->_username])->one();
        ...
        if($this->_type && $UserRecord->pwd != $this->_password) {
            $this->errorCode = -5;
